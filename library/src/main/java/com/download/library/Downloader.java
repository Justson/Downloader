...
import static com.download.library.FileComparator.COMPARE_RESULT_REDOWNLOAD_CONVER;
...
public class Downloader extends AsyncTask<Void, Integer, Integer> implements IDownloader<DownloadTask>, ExecuteTask {
							...
							if (downloadTask.getFile().length() > 0 && !isEncodingChunked) {
								if (downloadTask.getFile().length() == contentLength) {
									...
									if (compareResult == COMPARE_RESULT_SUCCESSFUL) {
										...
										return SUCCESSFUL;
									} else if (compareResult == COMPARE_RESULT_REDOWNLOAD_CONVER) {
										downloadTask.getFile().delete();
										downloadTask.getFile().createNewFile();
									} else {
										String fileName = "(" + new File(downloadTask.getFile().getParent()).list().length + ")" + downloadTask.getFile().getName();
										...
										Runtime.getInstance().log(TAG, "rename download , new file name:" + renameTarget.getName());
										if (!success) {
											downloadTask.getFile().delete();
											downloadTask.getFile().createNewFile();
										} else {
	...
	public boolean download(DownloadTask downloadTask) {
		DownloadTask cloneDownloadTask = downloadTask.clone();
		cloneDownloadTask.mId = downloadTask.mId;
		return downloadInternal(cloneDownloadTask);
	}
